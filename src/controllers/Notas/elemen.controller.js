...
const {convertirNum, calcularPromedioInsumos, calcPromInsFin, calcPromInsFinCuartos} = promElement()
...
async function primerIngresoNotas(idcurso, idmatricula, data) {
  try {
    ...
    const proAB = calcPromInsFin(pr3[0], pr3[1], pr3[2])
    data.notas['ppa'] = ppa
    data.notas['ppb'] = ppb
    ...
    const proCD = calcPromInsFin(pr6[0], pr6[1], pr6[2])
    data.notas['ppc'] = ppc
    data.notas['ppd'] = ppd
    ...
    const pr9 = convertirNum(ppe, ppf, Dto.pry3, '0')
    const proEF = calcPromInsFin(pr9[0], pr9[1], pr9[2])
    data.notas['ppe'] = ppe
    data.notas['ppf'] = ppf
...
async function primerIngresoCuarto(idcurso, idmatricula, data) {
  try {
    ...
    const pr2 = convertirNum(Dto.b1, Dto.b2, Dto.b3, Dto.b4)
    const ppb = calcularPromedioInsumos(pr2[0], pr2[1], pr2[2], pr2[3])
    const pr3 = convertirNum(ppa, ppb, Dto.pry1, Dto.exa1)
    const proAB = calcPromInsFinCuartos(pr3[0], pr3[1], pr3[2], pr3[3])
    data.notas['ppa'] = ppa
    data.notas['ppb'] = ppb
    data.notas['proAB'] = proAB
    ...
    const pr5 = convertirNum(Dto.d1, Dto.d2, Dto.d3, Dto.d4)
    const ppd = calcularPromedioInsumos(pr5[0], pr5[1], pr5[2], pr5[3])
    const pr6 = convertirNum(ppc, ppd, Dto.pry2, Dto.exa2)
    const proCD = calcPromInsFinCuartos(pr6[0], pr6[1], pr6[2], pr6[3])
    data.notas['ppc'] = ppc
    data.notas['ppd'] = ppd
    data.notas['proCD'] = proCD
    ...
    const pr8 = convertirNum(Dto.f1, Dto.f2, Dto.f3, Dto.f4)
    const ppf = calcularPromedioInsumos(pr8[0], pr8[1], pr8[2], pr8[3])
    const pr9 = convertirNum(ppe, ppf, Dto.pry3, Dto.exa3)
    const proEF = calcPromInsFinCuartos(pr9[0], pr9[1], pr9[2], pr9[3])
    data.notas['ppe'] = ppe
    data.notas['ppf'] = ppf
    data.notas['proEF'] = proEF
...
async function actualizarIngresoCuarto(idcurso, idmatricula, fkmateria, data) {
  try {
    ...
    const final = convertirNum(proAB, proCD, proEF, Dto.pytf)
    const final2 = calcPromInsFinCuartos(final[0], final[1], final[2], final[3])
